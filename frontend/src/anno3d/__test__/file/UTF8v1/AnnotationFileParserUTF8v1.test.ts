...
import { ParserUtf8Helper } from "~anno3d/v1/utf8/ParserUtf8";
...
describe("AnnotationFileParserUTF8v1", () => {
	let labels: Label[];
	let lines: string[][];
	let parser: ParserUtf8Helper;

	beforeEach(() => {
		labels = createLabels(2);
		lines = [["format UTF8", "version 1.0", "count 10"]];
		parser = new ParserUtf8Helper(labels);
	});
