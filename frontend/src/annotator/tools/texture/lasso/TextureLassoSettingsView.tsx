import { useI18nContext } from "i18n/i18n-react";
import { ColorSettingView } from "~ui/annotator/components/settings/ColorSettingView";
import { ToolHeading } from "~ui/annotator/components/settings/SettingView";
import { TEXTURE_LASSO_SETTINGS } from "./TextureLasso";

/**
 * The button component to access the quick setting of the TextureLasso
 *
 * @param props the component props
 * @returns the settings component
 */
export function TextureLassoSettingsView() {
	const { LL } = useI18nContext();

	return (
		<>
			<ToolHeading
				toolName={LL.LASSO()}
				annotationMode={LL.TRIANGLE_MESH_TEXTURE()}
			></ToolHeading>
			<ColorSettingView
				name={LL.LASSO_COLOR()}
				description={LL.LASSO_COLOR_DESC()}
				setting={TEXTURE_LASSO_SETTINGS.lineColor}
			></ColorSettingView>
		</>
	);
}
