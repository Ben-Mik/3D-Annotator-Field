import { Vector2, type Line3, type Vector3 } from "three";
 ...
 * The bounding box is defined by two points: the minimum and maximum points
 * in the x and y dimensions.
 *
 * @param points - An array of Vector2 points.
 * @returns A tuple containing two Vector2 points representing the bounding box.
 * @throws Error if no points are provided.
 */
export function getBoundingBox(points: Vector2[]): [Vector2, Vector2] {
	if (points.length === 0) {
		throw new Error("No points provided to calculate bounding box.");
	}
