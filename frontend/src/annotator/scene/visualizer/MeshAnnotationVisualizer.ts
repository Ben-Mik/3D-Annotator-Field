 ...
 */
export class MeshAnnotationVisualizer extends BaseVisualizer {
	 ...
	 */
	constructor(scene: Scene<Mesh>, labelManager: LabelManager) {
	...
	private visualizeWithTexture({ label, data }: LabeledAnnotationData) {
		...
		*/
		...
		for (let i = 0; i < data.length; i++) {
			...
			colorAttribute.set(colorVector, colorIndex);
		}
	...
	protected visualizeAllWithAnnotationsLUT(data: AnnotationsLUT): void {
		...
		if (colorAttribute.itemSize === 3) {
			for (let faceIndex = 0; faceIndex < data.length; faceIndex++) {
				const label = labelLUT[data[faceIndex]]!;
				const labelColor = label.color.floatValues;
				...
				const resultingColor = [
					labelColor[0] * newColorPart +
						this.originalColors![faceIndex * 9] * originalColorPart,
					labelColor[1] * newColorPart +
						this.originalColors![faceIndex * 9 + 1] *
							originalColorPart,
					labelColor[2] * newColorPart +
						this.originalColors![faceIndex * 9 + 2] *
							originalColorPart,
				];
